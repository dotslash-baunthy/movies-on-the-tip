// List a single favourites
// Server exposed /favourite:id endpoint
...
import { getFavouriteById } from "../../services/SingleMovie";

const FavouritesDetails = (props: RouteComponentProps<{ id: string }>) => {
    ...
    useEffect(() => {
        const fetchMovie = async () => {
            try {
                const data = await getFavouriteById(props.match.params.id)
                setMovie(data)
                setStatus("LOADED")
            }
            catch (errormsg: any) {
