// List a single movies in theatres
// Server exposed /movies-in-theatres:id endpoint
...
import { getMoviesInTheatresById } from "../../services/SingleMovie";

const MoviesInTheatresDetails = (props: RouteComponentProps<{ id: string }>) => {
    ...
    useEffect(() => {
        const fetchMovie = async () => {
            try {
                const data = await getMoviesInTheatresById(props.match.params.id)
                setMovie(data)
                setStatus("LOADED")
            }
            catch (errormsg: any) {
...
export default MoviesInTheatresDetails;